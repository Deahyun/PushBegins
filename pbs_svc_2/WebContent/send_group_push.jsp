...
$(document).ready( function() {
	...
	$("#btn_send_push").click( function() {
		...
		if ( group_id.length == 0 || user_id.length == 0 || push_name.length == 0 || push_alert.length == 0 || push_message.length == 0 ) {
		...
		//
		json_data["io_kind"] = "send_group_push_req";
	...
	$.ajax({
		...
		success: function(response) {
			...
			if ( result == "yes" ) {
				$("#message").html("<font color=blue>group push 를 전송했습니다.</font>");
				setTimeout(function() {
					$("#push_alert").val("");
					$("#push_message").val("");
					$("#message").html("");
				...
				$("#message").html("<font color=red>group push 를 전송중 오류가 발생 했습니다.</font>");
				setTimeout(function() {
					$("#message").html("");
				}, 3000);
			}
...
<!-- <input type="text" id="push_message" /> -->
	<form id="form1" action="Listener" method="post">
		...
		<fieldset>
			...
			<input type="button" value="group push 보내기" id="btn_send_push" />
		</fieldset>
	</form>
